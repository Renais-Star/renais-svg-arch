...
- For connections that would cross components, use two-segment (L-shaped) paths. **Do not use `<marker>`** (PPT drops it). Draw the path with a `<path>`, then add an explicit arrow triangle with `<polygon>` at the endpoint:
  ```svg
  <!-- L-shaped path ending at (midX, y2); arrow points down -->
  <path d="M x1,y1 L midX,y1 L midX,y2-9" fill="none" stroke="#64748b" stroke-width="1.5"/>
  <polygon points="midX-6,y2-11 midX,y2 midX+6,y2-11" fill="#64748b"/>
  ```
